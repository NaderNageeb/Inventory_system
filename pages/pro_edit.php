...
<?php
  ...
  $query = 'SELECT * from product p ,category c WHERE p.CATEGORY_ID=c.CATEGORY_ID and PRODUCT_ID ='.$_GET['id'];
  $result = mysqli_query($db, $query) or die(mysqli_error($db));
...
?>
  ...
  <center><div class="card shadow mb-4 col-xs-12 col-md-8 border-bottom-primary">
            ...
            <form role="form" method="post" action="pro_edit1.php">
              ...
              <option value='<?php echo $Did ;  ?>'  selected><?php echo $D; ?></option>

            <?php $sql = "SELECT * FROM category ";
      $result = mysqli_query($db, $sql) or die ("Bad SQL: $sql"); 
         while ($rows = mysqli_fetch_assoc($result)) { ?>
         <option value='<?php echo $rows['CATEGORY_ID'];  ?>'><?php echo $rows['CNAME'];  ?></option>
<?php } ?>
